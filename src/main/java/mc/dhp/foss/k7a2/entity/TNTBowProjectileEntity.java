...
import mc.dhp.foss.k7a2.procedures.TNTBowProjectileHitsLivingEntityProcedure;
...
public class TNTBowProjectileEntity extends AbstractArrow implements ItemSupplier {
	public static final ItemStack PROJECTILE_ITEM = new ItemStack(K7a2ModItems.TNT_ARROW.get());

	public TNTBowProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(K7a2ModEntities.TNT_BOW_PROJECTILE.get(), world);
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		TNTBowProjectileHitsLivingEntityProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		TNTBowProjectileHitsLivingEntityProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static TNTBowProjectileEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
	...
	public static TNTBowProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		TNTBowProjectileEntity entityarrow = new TNTBowProjectileEntity(K7a2ModEntities.TNT_BOW_PROJECTILE.get(), entity, world);
	...
	public static TNTBowProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		TNTBowProjectileEntity entityarrow = new TNTBowProjectileEntity(K7a2ModEntities.TNT_BOW_PROJECTILE.get(), entity, entity.level());
