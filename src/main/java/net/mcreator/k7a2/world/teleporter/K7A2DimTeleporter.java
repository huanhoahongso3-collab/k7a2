...
package net.mcreator.k7a2.world.teleporter;
...
import net.mcreator.k7a2.init.K7a2ModBlocks;
...
public class K7A2DimTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("k_7_a_2_dim_portal", Vec3i::compareTo, 300);
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(K7a2ModBlocks.K_7_A_2_DIM_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register("k_7_a_2_dim_portal", poiType);
	...
	public K7A2DimTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
			...
			for (int i3 = -1; i3 < 2; ++i3) {
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0 ? K7a2ModBlocks.K_7_A_2_PORTAL_BLOCK.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, K7a2ModBlocks.K_7_A_2_PORTAL_BLOCK.get().defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = K7a2ModBlocks.K_7_A_2_DIM_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = K7A2DimPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(), entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return K7A2DimPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity, entity.getDeltaMovement(), entity.getYRot(), entity.getXRot());
